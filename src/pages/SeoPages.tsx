...
import { CONTENT_PAGES_SELECT, ContentPage, findContentPage, mapContentPageRow } from '../lib/contentPages';
...
function useContentPages(type: 'service' | 'knowledge') {
  ...
  useEffect(() => {
    ...
    supabase
      ...
      .then(({ data }) => {
        if (!cancelled) setPages((data ?? []).map(mapContentPageRow));
      });
    return () => {
      cancelled = true;
    };
...
function useAllServicePages() {
  return useContentPages('service');
}
...
function ContentDetail({ page, kind, servicePages }: { page: ContentPage; kind: 'service' | 'knowledge'; servicePages: ContentPage[] }) {
  ...
  const related = page.relatedServices.map((slug) => {
    const service = findContentPage(servicePages, slug);
...
export function ServicePage() {
  ...
  const pages = useContentPages('service');
  if (!pages) return null;
  const page = findContentPage(pages, slug);
  return page ? <ContentDetail page={page} kind="service" servicePages={pages} /> : <Navigate to="/dich-vu" replace />;
}

export function KnowledgePage() {
  const { slug } = useParams();
  const knowledgePages = useContentPages('knowledge');
  const servicePages = useAllServicePages();
  if (!knowledgePages || !servicePages) return null;
  const page = findContentPage(knowledgePages, slug);
  return page ? <ContentDetail page={page} kind="knowledge" servicePages={servicePages} /> : <Navigate to="/kien-thuc" replace />;
}

export function ProjectsPage() {
  const path = '/du-an';
  const description = 'Dự án tiêu biểu được DHG Transport công bố và cập nhật từ hệ thống quản lý dự án.';
  return <SiteShell><SEO title="Dự án vận chuyển | DHG Transport" description={description} path={path} structuredData={[webPageSchema('Dự án vận chuyển', description, path), breadcrumbSchema([{ name: 'Trang chủ', path: '/' }, { name: 'Dự án', path }])]} /><Breadcrumbs items={[{ label: 'Dự án' }]} /><h1 className="text-4xl sm:text-5xl font-black font-heading">Dự án vận chuyển</h1><p className="mt-5 text-lg leading-8 text-slate-700">Các dự án tiêu biểu được hiển thị trên trang chủ được quản lý từ hệ thống dự án. Liên hệ DHG để trao đổi về nhu cầu có đặc tính tương tự.</p><RelatedContent links={[{ to: '/dich-vu/van-chuyen-hang-sieu-truong-sieu-trong', label: 'Dịch vụ vận chuyển hàng siêu trường siêu trọng' }, { to: '/#projects', label: 'Xem dự án đang công bố trên trang chủ' }, { to: '/#contact', label: 'Yêu cầu tư vấn dự án' }]} /></SiteShell>;
}
