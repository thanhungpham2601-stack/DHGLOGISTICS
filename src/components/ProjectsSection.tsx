...
export function ProjectsSection({ onOpenQuote }: ProjectsSectionProps) {
  ...
  return (
    <section id="projects" className="relative py-20 bg-[#f0f2f5] border-b border-[#e1e3e9]">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        {loading ? (
          ...
          <div className="text-sm text-slate-500 text-center py-10 border border-dashed border-[#dbdfe5] rounded-none">
        ...
        <StaggerGroup className="grid grid-cols-1 md:grid-cols-2 gap-6">
          {projects.map((item) => (
            <StaggerItem
              key={item.id}
              className="bg-[#eaebf0] rounded-none border border-[#dbdfe5] hover:border-[#1ba8e8] overflow-hidden transition-all duration-300 hover:shadow-2xl hover:shadow-[#0b6fa8]/10 group flex flex-col justify-between cursor-pointer"
            >
              <div
              ...
              >
              {/* Project Image */}
              <div className="relative h-60 w-full overflow-hidden bg-[#eeeff3]">
                <img
                  src={item.image}
                  alt={item.title}
                  ...
                  loading="lazy"
                />
                <div className="absolute inset-0 bg-gradient-to-t from-[#eaebf0] via-transparent to-black/30" />
                
                {/* Client Category Badge */}
                <div className="absolute top-4 left-4 px-3 py-1 rounded-none bg-[#ecedf1]/90 backdrop-blur-md border border-[#d4d6df] text-xs font-bold text-[#0b6fa8]">
                  {item.clientType}
                </div>

                {/* Weight Tag */}
                <div className="absolute top-4 right-4 px-3 py-1 rounded-none bg-[#1ba8e8] text-white text-xs font-black font-mono shadow-md">
                  {item.weight}
                </div>
              </div>
              ...
              <div className="p-6 space-y-4 flex-1 flex flex-col justify-between">
                <div>
                  <h3 className="text-xl font-bold text-slate-900 uppercase group-hover:text-[#0b6fa8] transition-colors leading-snug">
                    {item.title}
                  </h3>

                  <div className="mt-3 space-y-2 text-xs text-slate-700">
                    <div className="flex items-center gap-2">
                      <MapPin className="w-3.5 h-3.5 text-[#0b6fa8] shrink-0" />
                      <span className="font-semibold text-slate-800">Tuyến đường:</span>
                      <span>{item.route}</span>
                    </div>

                    <div className="flex items-center gap-2">
                      <Layers className="w-3.5 h-3.5 text-[#0b6fa8] shrink-0" />
                      <span className="font-semibold text-slate-800">Kích thước:</span>
                      <span className="font-mono">{item.dimension}</span>
                    </div>
                  </div>

                  <p className="text-xs text-slate-500 mt-3 p-3 rounded-none bg-[#e5e7ed] border border-[#dde1e7] leading-relaxed">
                    <span className="font-bold text-slate-800">Điểm then chốt: </span>
                    {item.highlight}
                  </p>
                </div>

                <div className="mt-4 pt-4 border-t border-[#dfe2e8] flex items-center justify-between">
                  <span className="text-xs font-mono text-[#0b6fa8] flex items-center gap-1">
                  ...
                  <button
                    ...
                    className="text-xs font-bold text-slate-900 hover:text-[#0b6fa8] flex items-center gap-1"
                  >
      ...
      {activeCase && (
        ...
        >
          <div
            ...
            className="bg-[#eaebef] border border-[#d4d7e0] rounded-none max-w-2xl w-full max-h-[90vh] overflow-y-auto shadow-2xl"
          >
            {/* Hero Image */}
            <div className="relative h-56 sm:h-72 w-full overflow-hidden bg-[#eeeff3] rounded-none">
              <img
              ...
              />
              <div className="absolute inset-0 bg-gradient-to-t from-[#eaebef] via-transparent to-black/20" />

              <button
                onClick={() => setActiveCase(null)}
                className="absolute top-4 right-4 p-2 rounded-none bg-[#eaebef]/90 backdrop-blur-sm text-slate-700 hover:text-slate-900 border border-[#d9dce2]"
              >
                <X className="w-5 h-5" />
              </button>
              ...
              <div className="flex gap-2 overflow-x-auto px-6 sm:px-8 pt-4">
                {activeCase.gallery.map((img, idx) => (
                  <button
                    ...
                    className={`relative shrink-0 w-20 h-16 rounded-none overflow-hidden border-2 transition-all ${
            ...
            <div className="p-6 sm:p-8 space-y-6">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                <div className="flex items-start gap-2.5 text-xs text-slate-800 bg-[#e6e7ed] p-3 rounded-none border border-[#dde0e6]">
                  <Weight className="w-4 h-4 text-[#0b6fa8] shrink-0 mt-0.5" />
                  <div>
                ...
                <div className="flex items-start gap-2.5 text-xs text-slate-800 bg-[#e6e7ed] p-3 rounded-none border border-[#dde0e6]">
                  <Layers className="w-4 h-4 text-[#0b6fa8] shrink-0 mt-0.5" />
                  <div>
                ...
                <div className="flex items-start gap-2.5 text-xs text-slate-800 bg-[#e6e7ed] p-3 rounded-none border border-[#dde0e6] sm:col-span-2">
                  <MapPin className="w-4 h-4 text-[#0b6fa8] shrink-0 mt-0.5" />
                  <div>
              ...
              <div>
                <h4 className="text-xs font-bold text-[#0b6fa8] uppercase tracking-wider font-mono mb-2 flex items-center gap-2">
              ...
              <div className="pt-2 flex items-center justify-between border-t border-[#dfe2e8] pt-4">
                <span className="text-xs font-mono text-[#0b6fa8] flex items-center gap-1">
                  <CheckCircle2 className="w-3.5 h-3.5" />
                  <span>Nghiệm thu an toàn 100%</span>
                </span>
                <button
                  ...
                  className="px-5 py-2.5 rounded-none bg-[#1ba8e8] hover:bg-[#3fc1ff] text-white font-bold text-xs tracking-wider uppercase shadow-lg shadow-[#0b6fa8]/20"
                >
